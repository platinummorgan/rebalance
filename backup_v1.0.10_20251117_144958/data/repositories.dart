...
class RepositoryService {
  ...
  static Future<void> initialize() async {
    ...
    // If we detected encryption errors, we need to recover by clearing
    // corrupted data and regenerating the encryption key
    if (encryptionError) {
      debugPrint('[Repository] Recovering from encryption error...');
      try {
        // Delete corrupted encryption key
        await _secureStorage.delete(key: _encryptionKeyKey);

        // Delete all corrupted box files
        ...
        debugPrint('[Repository] Deleted corrupted data, reinitializing...');

        // Recursive call to reinitialize with fresh encryption key
        return await initialize();
      } catch (recoveryError) {
        debugPrint('[Repository] Recovery failed: $recoveryError');
        rethrow;
      }
    }
